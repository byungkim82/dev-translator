...
- **Phase 2 상세 설계 완료**([§11](./TM-as-you-type-design.md)): gap 5개 확정 — ① `/api/similar` 재사용(신규 라우트 불필요, 이미 `is_favorite`+`similarity` 반환) ② `TmPanel` UX(디바운스 500ms·즐겨찾기만 예시 체크) ③ `exampleIds` → `fetchExamplesByIds` id 주입(임베딩 0) ④ ⚠️W9 정합성 = `had_examples` 불리언 컬럼(0006)+예시 force-fresh ⑤ 표시·주입 단일 컷오프 0.68. PR 분할: **PR-a 백엔드(무 UI, 무회귀 선행)** → **PR-b 프론트**. 결정 A~E 사인오프(§11.8). ✅ **PR-a 구현 완료** — 0006 마이그레이션(`had_examples`) + `findCachedTranslation` `had_examples=0` 필터 + `fetchExamplesByIds`(id 주입, 임베딩 0) + 라우트 `exampleIds` 수용(없으면 무회귀)·예시요청 캐시 스킵 + 유닛 테스트(총 102) · lint·tsc·`next build` 통과. **다음 = PR-b 프론트**(`TmPanel`·디바운스 TM 조회·`exampleIds` 전달).
