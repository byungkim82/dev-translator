...
> **상태:** ✅ **완료** — PR-a·PR-b(`5e2ceb0`·`7c84b45`) main 머지 + 배포·수동 E2E 검증 통과 (2026-07-05). 설계 확정(사인오프) · 리뷰 2회 반영(rev3). 131 테스트 통과.
> **작성:** 2026-07-03
...
### 5.0 시작 체크리스트
- [x] `docs/IMPROVEMENTS.md`: F11 상태 🔲→🚧→✅, 대시보드 행 갱신, F11 섹션에 본 문서 링크 추가, 완료 로그 이동.
...
**배포 후 수동 E2E 체크리스트(PR-b 후) — ✅ 전량 통과 (2026-07-05):**
- [x] "영어 → 한국어" 토글 → 스타일 셀렉터 사라지고 라벨/placeholder 변경.
- [x] 영어 문단 입력·Enter → 한국어가 **스트리밍**으로 렌더(첫 토큰 즉시), 커서 표시.
- [x] 영어 기술용어(deploy, PR, staging)가 한국어 출력에서 **영어로 보존**.
- [x] 읽기 중 TM 패널 **미표시**, Network 탭에 `/api/similar` 호출 **없음**, `/api/read`만 호출.
- [x] 읽기 결과에 즐겨찾기 **비활성**, 복사 버튼은 한국어 복사 동작.
- [x] 한→영으로 되돌리면 기존 동작(스트리밍·TM 패널·즐겨찾기·자동복사) **그대로**.
- [x] 긴 영어 입력으로 잘림 경고(truncated) 토스트 확인.
