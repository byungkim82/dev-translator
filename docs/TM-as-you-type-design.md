...
> **상태:** ✅ Phase 1·2 완료 + **프로덕션 배포·QA 통과 (2026-06-30)** (§11: PR-a 백엔드 + PR-b 프론트 as-you-type TM 패널) · 컷오프 0.68 실사용 미세조정만 남음 · Phase 3 보류
> **작성:** 2026-06-26
...
## 6. 실측으로만 검증되는 것 (플래그) — 대부분 해소됨
1. ✅ **bge-m3의 *우리 짧은-문장 패러프레이즈* 한국어 품질** — 스팟체크(§8) + 프로덕션 QA에서 강한 패러프레이즈(`리뷰 부탁드려요`↔`한번 봐주실 수 있나요?`) **83% 매치** 확인.
2. 🟡 **bge-m3용 유사도 임계값** — 0.68로 재보정(§8). QA에서 강한 매치 0.83(여유 0.15)·오탐 0. **실사용 며칠 후 오탐 빈도로 최종 미세조정만 남음**(`EDGE_SIMILARITY_THRESHOLD`).
3. ✅ **Workers AI 엣지 임베딩 지연** — 스팟체크 ~254ms(6문장), QA에서 as-you-type 체감 지연 없음 + `ctx.waitUntil` 백그라운드 기록이 응답 무차단 확인.
4. (Phase 3) **인-브라우저 임베딩 지연**(WASM/WebGPU) — 기기/브라우저 의존. Phase 3 보류라 미측정.
5. ✅ **Qwen3-Embedding-0.6B vs bge-m3** — 스팟체크에서 분리 미미 + 가장 느림(~2382ms) → qwen3 드롭(§8).
...
- **Phase 1 — ✅ 구현·배포·QA 완료.** 마이그레이션 0005 + 임베딩 핫패스 분리(`ctx.waitUntil` 백그라운드 기록) + bge-m3 게이팅(0.68) + 멱등 백필(1068행) + 유닛 테스트. §9 참고. **프로덕션 QA 통과(2026-06-30):** `ctx.waitUntil` 백그라운드 기록이 실 Workers에서 동작(§6 flag 3), 첫 토큰 즉시(1.3s 블록 해소).
- **Phase 2 — ✅ 상세 설계(§11) + 구현(PR-a·b) + 배포·QA 완료.** gap 5개(§11) 전부 확정·구현. **프로덕션 QA 통과(2026-06-30):** as-you-type 패널(83% 매치), 즐겨찾기만 예시 체크박스, `exampleIds` 전송, **⚠️W9 캐시 정합성 트랩 검증**(plain→예시→plain 시 정확히 2행, 예시본 격리 = `had_examples`). **남은 건 컷오프 0.68 실사용 미세조정만.**
